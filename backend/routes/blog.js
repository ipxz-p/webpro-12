...
router.post("/blogs", upload.array("myImage", 5), isLoggedIn, async function (req, res, next) {
  ...
  try {
    let results = await conn.query(
      ...
      [title, content, status, pinned, create_by_id]
    );
...
router.put("/blogs/:id", upload.array("myImage", 5), isLoggedIn, blogOwner, async function (req, res, next) {
  // Your code here
  const file = req.files;
  let pathArray = []
